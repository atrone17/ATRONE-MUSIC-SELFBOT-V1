...
\`${this.prefix}autoplay\` - Toggle Atrone AI recommendations
...
`;
    ...
    async ghostyNowPlayingCommand(message) {
        ...
        const requesterText = track.requester?.id ? `<@${track.requester.id}>` : 'ATRONE AI V1';
...
`;
    ...
    async ghostyAutoplayCommand(message) {
        ...
        message.channel.send(`↻ **Autoplay:** ${statusText}\n-# Powered by Atrone AI V1 | Oblivion Developement`);
